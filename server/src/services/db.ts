...
import { 
  INITIAL_USERS, INITIAL_INSTITUTIONS, INITIAL_CONTACTS, 
  INITIAL_TEAMS, INITIAL_PARTICIPANTS, INITIAL_PAYMENTS, 
  INITIAL_VERIFICATIONS, INITIAL_HOSPITALITY, INITIAL_FIXTURES, 
  INITIAL_RESULTS, INITIAL_AUDIT_LOGS, INITIAL_BANK_PAYMENTS 
...
export const addAuditLog = async (user: string, role: string, action: string, details: string): Promise<any> => {
...
const getPostgresConnectionStr = (dbUrl: string): string => {
...
export const initDb = async (): Promise<void> => {
  ...
  try {
    ...
    for (const e of EVENTS_CATALOG) {
      await dbQuery(
        `INSERT INTO events (id, name, category, type, min_team_size, max_team_size, registration_fee, description, rules, eligibility)
        ...
        [e.id, e.name, e.category, e.type, e.minTeamSize, e.maxTeamSize, e.registrationFee, e.description, JSON.stringify(e.rules), e.eligibility]
      );
    }
    ...
    for (const u of INITIAL_USERS) {
      await dbQuery(
        ...
        [u.id, u.username, u.name, u.role, u.email, u.eventId || null]
      );
    }
