    ...
    try {
        ...
        const results = institutions.map((inst) => {
            ...
            const instTeams = teams
                .filter((t) => t.institution_id === inst.id)
                .map((t) => ({
                id: t.id,
            ...
            const instParts = participants
                .filter((p) => p.institution_id === inst.id)
                .map((p) => ({
                id: p.id,
            ...
            const paymentObj = payments.find((p) => p.institution_id === inst.id);
            ...
            const verObj = verifications.find((v) => v.institution_id === inst.id);
    ...
    try {
        ...
        const assignedCount = await withTransaction(async (client) => {
            ...
            if (verCheck.rows.length === 0) {
                ...
                await client.query(`INSERT INTO verifications (id, registration_id, institution_id, verified_by, verified_at, status, remarks)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [verId, inst.registration_id, institutionId, verifierName || 'Registration Team', new Date().toISOString(), 'VERIFIED', remarks]);
            }
            else {
                await client.query(`UPDATE verifications SET status = 'VERIFIED', verified_at = $1, verified_by = $2, remarks = $3 WHERE institution_id = $4`, [new Date().toISOString(), verifierName || 'Registration Team', remarks, institutionId]);
            }
            // 2. Update Teams status to VERIFIED
            await client.query(`UPDATE teams SET status = 'VERIFIED' WHERE institution_id = $1`, [institutionId]);
            // 3. Update Payment status to SUCCESS
            await client.query(`UPDATE payments SET status = 'SUCCESS' WHERE institution_id = $1`, [institutionId]);
    ...
    try {
        ...
        await withTransaction(async (client) => {
            ...
            if (verCheck.rows.length === 0) {
                const verId = `ver_${Date.now()}`;
                await client.query(`INSERT INTO verifications (id, registration_id, institution_id, verified_by, verified_at, status, remarks)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [verId, inst.registration_id, institutionId, verifierName || 'Registration Team', new Date().toISOString(), 'REJECTED', remarks]);
            }
            else {
                await client.query(`UPDATE verifications SET status = 'REJECTED', remarks = $1, verified_at = $2, verified_by = $3 WHERE institution_id = $4`, [remarks, new Date().toISOString(), verifierName || 'Registration Team', institutionId]);
            }
            // 2. Update Teams status
            await client.query(`UPDATE teams SET status = 'REJECTED' WHERE institution_id = $1`, [institutionId]);
            // 3. Update Participants status
            await client.query(`UPDATE participants SET verification_status = 'REJECTED' WHERE institution_id = $1`, [institutionId]);
            // 4. Audit Log
            const logId = `log_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
            await client.query(`INSERT INTO audit_logs (id, timestamp, user_name, role, action, details)
         VALUES ($1, $2, $3, $4, $5, $6)`, [
