...
export const getCertificateData = async (req, res) => {
    try {
        ...
        const certificatesList = verifiedParticipants.map((part, idx) => {
            const inst = institutions.find((i) => i.id === part.institution_id);
            const event = events.find((e) => e.id === part.event_id);
            const result = results.find((r) => r.event_id === part.event_id);
            let certType = 'PARTICIPATION';
            if (result) {
                if (result.winner_team_id === part.team_id)
                    certType = 'WINNER';
                else if (result.runner_up_team_id === part.team_id)
                    certType = 'RUNNER_UP';
                else if (result.second_runner_up_team_id === part.team_id)
                    certType = 'SECOND_RUNNER_UP';
            }
