...
export const login = async (req, res) => {
    ...
    try {
        ...
        const user = users.find((u) => u.username.toLowerCase() === inputStr ||
        ...
        // Generate JWT token
        const token = jwt.sign({ id: user.id, username: user.username, role: user.role, name: user.name, eventId: user.event_id || user.eventId }, getJwtSecret(), { expiresIn: '12h' });
