...
export const getHospitalityData = async (req, res) => {
    try {
        // ONLY VERIFIED INSTITUTIONS & PARTICIPANTS appear in Hospitality Desk
        const instsRes = await dbQuery(`SELECT i.* FROM institutions i
...
export const updateHospitalityStatus = async (req, res) => {
    ...
    try {
        ...
        if (checkRes.rows.length === 0) {
            ...
            await dbQuery(`INSERT INTO hospitality (id, institution_id, arrival_status, arrival_time, accommodation_hall, food_preference, special_requirements, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
            ...
            await dbQuery(`UPDATE hospitality 
         SET arrival_status = $1, arrival_time = $2, accommodation_hall = $3, food_preference = $4, special_requirements = $5, updated_at = $6
         WHERE institution_id = $7`, [updatedArrivalStatus, updatedArrivalTime, updatedAccommodation, updatedFood, updatedSpecial, nowStr, institutionId]);
        ...
        await addAuditLog(user?.name || 'Hospitality Desk', 'hospitality_team', 'UPDATE_HOSPITALITY', `Updated arrival status to '${hospRecord.arrival_status}' for ${instName}`);
        const mappedHosp = {
            id: hospRecord.id,
