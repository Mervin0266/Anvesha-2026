...
export const HospitalityDashboard: React.FC = () => {
  ...
  const [arrivalStatus, setArrivalStatus] = useState<'PENDING' | 'ATTENDED'>('PENDING');
  ...
  const handleQuickStatusToggle = async (item: any) => {
    const newStatus = item.hospitality.arrivalStatus === 'ATTENDED' ? 'PENDING' : 'ATTENDED';
    try {
      const res = await apiFetch<{ success: boolean; message: string }>('/hospitality/update', {
        method: 'POST',
        body: JSON.stringify({
          ...
          accommodationHall: null,
          foodPreference: 'Veg',
          specialRequirements: item.hospitality.specialRequirements || '',
          user
        })
      });
      if (res.success) {
  ...
  const filtered = data.filter(item => 
    ...
    (item.chestNumbers && item.chestNumbers.some((c: string) => c.toLowerCase().includes(searchQuery.toLowerCase())))
  );
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto overflow-x-hidden pt-16 lg:pt-0">
        <Header title="Hospitality & Student Logistics Desk" subtitle="Monitor verified college delegations, volunteer status, student counts, and chest numbers." />
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-50 text-slate-600 font-serif border-b border-slate-200">
                      ...
                      <th className="p-3.5 font-bold">Chest Numbers</th>
                      <th className="p-3.5 font-bold">Volunteer Status</th>
                    ...
                    {filtered.map((item) => (
                      <tr key={item.institution.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-3.5 text-slate-700">
                          {item.chestNumbers && item.chestNumbers.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {item.chestNumbers.map((c: string) => (
                                <span key={c} className="px-2 py-0.5 bg-slate-100 border border-slate-200 rounded text-[10px] font-mono font-bold text-slate-700">
                                  {c}
                                </span>
                              ))}
                            </div>
                        ...
                        <td className="p-3.5">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                            item.hospitality.arrivalStatus === 'ATTENDED' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                          }`}>
                            {item.hospitality.arrivalStatus === 'ATTENDED' ? 'Attended' : 'Pending'}
                          </span>
                        </td>
                        <td className="p-3.5 text-right">
                          <div className="flex items-center justify-end space-x-2">
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm transition-all border ${
                                item.hospitality.arrivalStatus === 'ATTENDED'
                                  ? 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-105'
                                  : 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-105'
                              }`}
                            >
                              {item.hospitality.arrivalStatus === 'ATTENDED' ? 'Mark Pending' : 'Mark Attended'}
                            </button>
                            <button
                              onClick={() => {
                                setSelectedItem(item);
                                setArrivalStatus(item.hospitality.arrivalStatus === 'ATTENDED' ? 'ATTENDED' : 'PENDING');
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl border border-slate-200">
                ...
                <div className="space-y-4 text-xs">
                  <div>
                    <label className="block font-bold text-slate-800 mb-1">Volunteer Status *</label>
                    <select
                      value={arrivalStatus}
